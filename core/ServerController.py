...
class RemoteServer:
    ...
    def __get_service_list(self):
        command = '{} -plaintext {} list'.format(self.cmd, self.host)
    ...
    def get_method_list(self):
        ...
        for service in services:
            command = '{} -plaintext {} list {}'.format(self.cmd, self.host, service)
            output = subprocess.check_output(command, shell=True)
    ...
    def __get_message_schema(self, method):
        command = '{} -plaintext {} describe {}'.format(self.cmd, self.host, method)
        output = subprocess.check_output(command, shell=True)
    ...
    def get_message_template(self, method):
        ...
        command = '{} -plaintext -msg-template {} describe {}'.format(self.cmd, self.host, request)
        output = subprocess.check_output(command, shell=True)
        message_template = json.loads(output.decode('utf8').split('Message template:')[1].replace('\n', '').replace(' ', ''))
        return message_template

    def send_request(self, request, method):
        command = '{} -plaintext -d \'{}\' {} {}'.format(self.cmd, request, self.host, method)
        output = subprocess.check_output(command, shell=True)
        return json.loads(output.decode('utf-8'))

    def view_method_scheme(self, method):
        data = self.__get_message_schema(method)
        command_request = '{} -plaintext -msg-template {} describe {}'.format(self.cmd, self.host, data['request'])
        command_response = '{} -plaintext -msg-template {} describe {}'.format(self.cmd, self.host, data['response'])
